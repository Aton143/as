...
size_t GetIPAddressString(ADDRINFOW *Address, void *Buffer, size_t BufferByteCount)
{
  return(-1);
}
...
int32_t __cdecl main(int32_t, char **)
{
    ...
    // - The fonts will be rasterized at a given size (w/ oversampling) and stored int32_to a texture when calling ImFontAtlas::Build()/GetTexDataAsXXXX(), which ImGui_ImplXXXX_NewFrame below will call.
    ...
    SOCKET ClientSocket = INVALID_SOCKET;
    {
      ...
      ADDRINFOW *AddressResult = NULL;
      WSAResult = GetAddrInfoW(L"localhost", L"27015", &AddressHints, &AddressResult);
      ...
      for (ADDRINFOW *CurAddress = AddressResult; CurAddress != NULL; CurAddress = CurAddress->ai_next)
      {
        uint32_t SocketFlags = 0;
        ClientSocket = WSASocketW(CurAddress->ai_family,
                                  CurAddress->ai_socktype,
                                  CurAddress->ai_protocol,
                                  NULL,
                                  0,
                                  SocketFlags);
        ...
        // Connect to server.
        WSAResult = WSAConnect(ClientSocket,
                               CurAddress->ai_addr,
                               (int32_t) CurAddress->ai_addrlen,
                               NULL, NULL, NULL, NULL);
...
LRESULT WINAPI WndProc(HWND WindowHandle, UINT Message, WPARAM WParam, LPARAM LParam)
{
    ...
    switch (Message)
    {
      case WM_SIZE:
      {
        ...
        ResizeWidth  = (uint32_t) LOWORD(LParam);
        ResizeHeight = (uint32_t) HIWORD(LParam);
